...
class Stage3TartanPoseDataset(Dataset):
    def __init__(self, split: str, root: str | Path = STAGE3_TARTANVO_FEATURE_CACHE):
        ...
        if not self.index_path.is_file():
            raise FileNotFoundError(f"TartanVO pose cache index not found: {self.index_path}")
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        path = self.root / self.split / f"{row.sample_key}.pt"
        item = torch.load(path, map_location="cpu", weights_only=False)
        return {
            "pose": item["pose"].to(torch.float32),

...
def cache_split(split: str, feature: str, root: Path = STAGE3_TARTANVO_FEATURE_CACHE, overwrite: bool = False) -> None:
    ...
    with torch.inference_mode():
        for i in tqdm(range(len(dataset)), desc=f"cache tartanvo {feature} {split}"):
            ...
            if overwrite or not path.exists():
                ...
                cached = model.feature_sequence(video).squeeze(0).cpu().to(torch.float32)
                torch.save({
                    "feature": cached,
...
def main() -> None:
    ...
    cache_split(args.split, args.feature, overwrite=args.overwrite)
